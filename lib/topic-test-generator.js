...
/**
 * Prettify a snake_case subtype for display in a test name.
 * "algebra_polynomial"        -> "Algebra Polynomial"
 * "gk_misc"                   -> "GK Misc"   (2-letter words become ALL CAPS)
 * "arithmetic_profit_loss"    -> "Arithmetic Profit Loss"
 * Admin can inline-rename to override.
 */
export function prettifySubtype(s) {
    return String(s || '')
        .replace(/_/g, ' ')
        .split(/\s+/)
        .filter(Boolean)
        .map(w => {
            if (w.length <= 2 && /^[A-Za-z]+$/.test(w)) return w.toUpperCase();
...
export async function generateTopicTest(client, { exam_id, subtype, difficulty_level, user_id }) {
    ...
    // 3. Next "{Pretty Subtype} {letter}{N}" — counter per (subtype, level), global across exams
    const letter = LEVEL_LETTERS[level];
    const prettyName = prettifySubtype(subtype);
    const escSubtype = prettyName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const capturePattern = `^${escSubtype} ${letter}(\\d+)$`;
    const matchPattern   = `^${escSubtype} ${letter}\\d+$`;
    const numRes = await client.query(`
        ...
        WHERE test_type = 'TOPIC'
          ...
          AND name ~ $3
    `, [capturePattern, level, matchPattern]);
    const nextNum = numRes.rows[0].next_num;
    const mockName = `${prettyName} ${letter}${nextNum}`;
