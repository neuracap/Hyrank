...
// Section-code patterns where the section is mandatorily a specific
// language regardless of the paper's "primary" language. CGL/CHSL/etc.
// English section is English on both EN and HI papers; the same is true
// in reverse for Hindi-language sections in papers that have one.
const ENGLISH_SECTION_RE = /\b(ENGLISH|ENG|ENGLISH_LANGUAGE|ENGLISH_COMPREHENSION)\b/i;
const HINDI_SECTION_RE   = /\b(HINDI|HIN|HINDI_LANGUAGE)\b/i;

function classifySection(code) {
    if (!code) return 'other';
    if (ENGLISH_SECTION_RE.test(code)) return 'english_section';
    if (HINDI_SECTION_RE.test(code)) return 'hindi_section';
    return 'other';
}
...
//
// Optional 3rd argument `ctx` lets callers pass the pair's section_code
// so script-mismatch heuristics can skip language-locked sections
// (e.g. CGL ENGLISH stays in English on both EN and HI papers).
// =========================================================
export function checkBilingualPair(en, hi, ctx = {}) {
    const issues = [];
    ...
    const sectionType = classifySection(ctx.section_code);
    ...
    // Skip on language-locked sections (English/Hindi sections by design
    // have identical-language stems on both sides, so length ratio is 1×
    // anyway, but the script-based length comparison is also noisy when
    // both sides are in the same script).
    const enLen = (en.text || '').replace(/\s+/g, '').length;
    const hiLen = (hi.text || '').replace(/\s+/g, '').length;
    if (sectionType === 'other' && enLen >= 30 && hiLen >= 30) {
    ...
    // EXCEPT for English-language sections (CGL English, etc.) where the
    // Hindi-side row is mandatorily in English on the paper.
    if (sectionType !== 'english_section' && hi.text && hi.text.length >= 20 && !DEVANAGARI_RE.test(hi.text)) {
    ...
    // EXCEPT for Hindi-language sections where the EN-side row is
    // mandatorily in Hindi (rare but exists in some exam blueprints).
    if (sectionType !== 'hindi_section' && en.text && en.text.length >= 20) {
