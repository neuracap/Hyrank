...
/**
 * Section-wide target for PYQ-sourced picks (= 50% of SECTION_TOTAL).
 ...
 *
 * The cap equals the target so a section never carries MORE than ~half PYQs.
 * If a topic only has one source available, the picker takes it regardless.
 */
export const PYQ_TARGET_PER_SECTION = Math.floor(SECTION_TOTAL * 0.5);  // 12
export const PYQ_CAP_PER_SECTION = PYQ_TARGET_PER_SECTION;              // 12
...
export const SUBTYPE_PREFIXES = {
    // REASONING — patterns cover BOTH bank (hierarchical: 'series_number_series') and
    // PYQ (flat: 'series', 'classification', 'seating_arrangement'). Trailing '%' on
    // the topic name catches both bank's '<topic>_<variant>' and PYQ's bare '<topic>'.
    analogy:               ['analogy%'],
    odd_one_out:           ['odd_one_out%', 'classification%'],                  // PYQ uses 'classification'
    series:                ['series%', 'letter_series', 'alphabet_series',
                            'figure_series', 'visual_series'],                   // PYQ has bare 'series' (1352) + several variants
    coding_decoding:       ['coding_decoding%'],
    blood_relation:        ['blood_relation%'],
    direction_sense:       ['direction_sense%', 'direction%'],
    word_arrangement:      ['word_arrangement%', 'sitting_arrangement%',
                            'seating_arrangement%', 'ranking%',                   // PYQ uses 'seating' (US) and 'ranking_order'
                            'alphabetical_order', 'logical_order', 'dictionary_order'],
    syllogism:             ['syllogism%'],
    venn_diagram:          ['venn_diagram%'],
    mathematical_operation:['mathematical_operation%'],                          // catches PYQ plural 'mathematical_operations'
    missing_number:        ['missing_number%'],
    cube_dice:             ['cube_dice%', 'cube_and_dice%'],
    ...
    // QUANT — bank uses '<topic>_<variant>' (e.g. 'percentage_basic_…'); PYQ uses
    // 'arithmetic_<topic>' for arithmetic bucket (e.g. 'arithmetic_percentage'),
    // and bare names for advanced ('trigonometry', 'mensuration', 'geometry_2d').
    // Patterns end with '%' so both shapes are caught by the same prefix.
    arithmetic:    [
        'percentage_%',
        'profit_loss_%',
        'simple_interest_%',
        ...
        'arithmetic_%',  // catch-all — also picks up every PYQ arithmetic_<topic>
    ],
    applied:       [
        ...
        'arithmetic_time_speed_distance%', 'arithmetic_time_work%',  // PYQ shape for tsd / work-time
        ...
        'height_distance_%',
        'discount_%',
        'data_interpretation%',                                       // catches PYQ bare 'data_interpretation'
    ],
    advanced:      [
        'algebra%', 'geometry%', 'trigonometry%', 'mensuration%',
        'coordinate_geometry%', 'coordinate%',                        // % at end catches PYQ bare names too
    ],
    number_system: ['hcf_lcm%', 'number_system%', 'number%', 'arithmetic_number_system%'],
    // ENGLISH
