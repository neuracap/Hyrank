/**
 * SSC GD Constable mock-test spec (5 sections × 20 Q = 100 Q).
 *
 * Differences from CGL T1:
 *   - SECTION_TOTAL = 20 (not 25) — targets sum to 20 per section.
 *   - Adds a HINDI section (5th section) with its own subtype map + targets.
 *   - PYQ source restricted to GD + CGL (CHSL PYQs excluded — too difficult).
 *   - Difficulty profile leans easier (L1+L2 dominant).
 *
...
export const SECTION_CODES = ['REASONING', 'GA', 'QUANT', 'ENGLISH', 'HINDI'];
...
// 40% of 20 = 8 — matches CGL's percentage choice.
export const PYQ_TARGET_PER_SECTION = Math.floor(SECTION_TOTAL * 0.4);
