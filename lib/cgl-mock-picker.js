...
import {
    ...
    BUCKET_TOPICS, MAX_PER_TOPIC, PYQ_CAP_PER_SECTION,
    buildPlaceholderTemplates, placeholderCountsBySection,
} from './cgl-mock-spec.js';

// Per-section PYQ-budget tracker. Mutated as PYQ picks land; once depleted,
// the helpers below refuse any further PYQ candidate (treating them as
// effectively absent for the remainder of that section's picks).
function newPyqBudget() {
    return { used: 0, cap: PYQ_CAP_PER_SECTION };
}

function isBank(q) { return (q?.source_type || 'bank') === 'bank'; }

// Sort a candidate pool so bank rows come first within each (difficulty) tier.
// Used to bias every "pick first available" loop toward bank without changing
// the rest of the algorithm.
function bankFirst(arr) {
    return arr.slice().sort((a, b) => {
        const aBank = isBank(a) ? 0 : 1;
        const bBank = isBank(b) ? 0 : 1;
        return aBank - bBank;
    });
}
...
function pickFromBagBiased(candidates, n, { excludedIds, levelDeficit, pyqBudget = null }) {
    ...
    // Bank first within each difficulty bucket — PYQ falls through when bank is short.
    const byDifficulty = {
        1: bankFirst(shuffleStable(pool.filter(q => q.difficulty === 1))),
        2: bankFirst(shuffleStable(pool.filter(q => q.difficulty === 2))),
        3: bankFirst(shuffleStable(pool.filter(q => q.difficulty === 3))),
        4: bankFirst(shuffleStable(pool.filter(q => q.difficulty === 4))),
    };
    ...
    const pickOneAtLevel = (lvl) => {
        ...
        // First pass: fresh variation, respecting PYQ budget
        ...
        // Fallback: any row left in this bucket that's still acceptable.
        for (let i = 0; i < bucket.length; i++) {
            const q = bucket[i];
            if (acceptable(q)) {
                bucket.splice(i, 1);
...
function pickFromTopicBucket(candidates, n, opts) {
    ...
    // then re-sort so bank candidates come first within the topic. PYQ rows are
    // only reached after every bank row in the topic has been considered.
    for (const t of Object.keys(buckets)) {
        buckets[t] = bankFirst(shuffleStable(buckets[t], seed + (t.length * 31)));
    }
    ...
    const pickOneFromTopic = (t) => {
        ...
        if (!bucket || bucket.length === 0) return false;
        // Prefer the difficulty level with largest remaining deficit; if multiple,
        // gentler ramp (lower level) breaks ties. Within a level, prefer bank rows
        // (bucket is bank-first sorted) and skip PYQ rows if the section's PYQ
        // budget is depleted.
        ...
        for (const lvl of levelsAvail) {
            const idx = bucket.findIndex(q => q.difficulty === lvl && acceptable(q));
            if (idx >= 0) {
                const q = bucket[idx];
                bucket.splice(idx, 1);
                recordPick(q, t);
...
function pickGroup(groups, groupType, excludedIds, opts = {}) {
    const { minSize = 0, maxSize = null, minPassageChars = 0, preferComposition = null } = opts;
...
function pickSection({
    ...
    const pyqBudget = newPyqBudget();   // section-wide PYQ cap (= 10 by default)
    ...
    if (pyqBudget.used > 0) {
        notes.push(`[${sectionCode}] used ${pyqBudget.used} PYQ pick(s) of ${pyqBudget.cap} allowed (${Math.round(pyqBudget.used / SECTION_TOTAL * 100)}% PYQ).`);
    }
