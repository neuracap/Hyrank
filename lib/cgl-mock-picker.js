...
import { getSpec } from './mock-spec-resolver.js';

// The picker reads spec constants through these module-scoped bindings, which
// are repopulated at the top of every buildMock() call from the resolved spec.
// Safe across requests because buildMock is fully synchronous (no await), so
// Node's single-threaded runtime guarantees no two specs are ever live at once.
let SECTION_CODES, SECTION_TOTAL, SECTION_SPEC, SUBTYPE_PREFIXES;
let DIFFICULTY_LEVELS, SECTION_DIFFICULTY_BASE;
let BUCKET_TOPICS, MAX_PER_TOPIC, PYQ_CAP_PER_SECTION, PYQ_TARGET_PER_SECTION;
let buildPlaceholderTemplates, placeholderCountsBySection;
...
// Pre-populate with CGL T1 so legacy callers that import the picker for side
// effects (or call it before buildMock first runs) see consistent values.
applySpec(getSpec('cgl-t1'));

// Per-section PYQ-budget tracker. `target` is the active 50% goal; the helpers
// PREFER PYQ candidates over bank within a topic as long as `used < target`.
// Once `used` hits `target` (= 12 for CGL/CHSL, 10 for GD), preference flips
// to bank for the remainder of the section. `cap` is the hard ceiling — when
// `used` reaches it, PYQ candidates are refused outright.
...
// `prefer` = 'pyq' when we're still chasing the 50% target; 'bank' once met.
...
function pickFromTopicBucket(candidates, n, opts) {
    ...
    const pickOneFromTopic = (t) => {
        ...
        // chasing — PYQ until the 50% target is met, bank after.
...
function pickSection({
    ...
    // PYQ target so the SECTION total still lands at ~50% PYQ instead of
    // overshooting (picker-PYQ + visual-PYQ).
 ...
 * }} args
 */
export function buildMock({ examKey, config, poolsBySection, groups, excludedIds, userBankTargets, difficultyProfile }) {
    // Repopulate the spec bindings before any picker logic runs.
    applySpec(getSpec(examKey));
    const notes = [];
