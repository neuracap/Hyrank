...
import { TARGET_SECTION_IDS } from '@/lib/cgl-mock-spec';

export const dynamic = 'force-dynamic';
export const maxDuration = 300; // ~75 questions × 5 fields × ~300ms each

/**
 * POST /api/mock-test/[id]/translate-hindi
 *
 * For a CGL T1 mock, translates every GA/REASONING/QUANT question's stem +
 * 4 options EN → HI via google-translate-api-x (with LaTeX + proper-noun
 * protection from lib/translate-helpers). Overwrites existing HI rows.
 *
...
export async function POST(req, { params }) {
    ...
    try {
        ...
        // 4. Translate + upsert HI for each question
        const counts = { processed: 0, failed: 0, by_section: {} };
        const errors = [];

        for (const r of qRes.rows) {
            const sectionCode = Object.keys(TARGET_SECTION_IDS).find(c => TARGET_SECTION_IDS[c] === r.exam_section_id) || '?';
