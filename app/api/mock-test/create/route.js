...
export async function POST(req) {
    ...
    try {
        const res = await db.query(`
            INSERT INTO mock_test (mock_test_id, exam_id, name, status, created_by, created_at, updated_at)
            VALUES (gen_random_uuid(), $1, $2, 'DRAFT', $3, NOW(), NOW())
            RETURNING mock_test_id, name
        `, [exam_id, name, user.id]);
