...
15. **Ask if the request belongs to this window** — When an instruction names a feature, page, file, or term that has no presence in this Hyrank repo (admin question-curation tool), don't guess and don't hunt forever. Ask the user whether the request was intended for this window or for a different repo (e.g. yoloprep, a library/subscription app, etc.). Examples of out-of-scope signals: subscription/billing copy, referral/promo flows, student-facing UI text that isn't on an admin page, public-app routing, payment processors. A quick "is this meant for Hyrank or another window?" saves both sides time.
