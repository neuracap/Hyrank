...
async function run() {
    ...
    try {
        ...
        const sessionRes = await client.query(`SELECT paper_session_id, language FROM paper_session WHERE paper_session_id = $1`, [paperSessionId]);
        ...
        const sectionRes = await client.query(`SELECT section_id, name, paper_session_id FROM exam_section WHERE paper_session_id = $1`, [paperSessionId]);
        ...
        // CTE logic from page.js (simplified)
        ...
        const query = `
            ...
            LIMIT 5;
        `;
        const fullRes = await client.query(query, [paperSessionId]);
