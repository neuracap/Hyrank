...
export default function ImageSolutionsBilingual({ papers }) {
    ...
    const handlePaperClick = async (paper) => {
        ...
        try {
            ...
            if (res.ok && data.pairs) {
                ...
                const opts = {}, subs = {}, solEn = {}, solHi = {}, results = {};
                for (const p of data.pairs) {
                    const label = p.correct_option_label || p.eng_solution_json?.correct_option_label;
                    if (label) opts[p.link_id] = label;
                    if (p.subtype) subs[p.link_id] = p.subtype;
                    if (p.eng_solution_json?.display_sections?.exam_craft?.approach) {
                        solEn[p.link_id] = p.eng_solution_json.display_sections.exam_craft.approach;
                    }
                    if (p.eng_solution_json?.display_sections?.exam_craft?.approach_hi) {
                        solHi[p.link_id] = p.eng_solution_json.display_sections.exam_craft.approach_hi;
                    }
                    if (p.eng_solution_json?.display_sections) {
                        results[p.link_id] = {
                            full_json: p.eng_solution_json,
    ...
    const handleTranslate = useCallback(async (linkId) => {
        ...
        if (!text) {
            setFeedback({ type: 'error', message: 'Write the English explanation first.' });
            return;
        }
    ...
    const handleGenerate = useCallback(async (pair) => {
        ...
        if (!answerLabel) {
            setFeedback({ type: 'error', message: 'Select the correct answer first.' });
            return;
        }
        setGeneratingId(linkId);
        setFeedback(null);
        try {
            const res = await fetch('/api/image-solutions-bilingual/generate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...
                    reviewer_solution_en: solutionsEn[linkId] || '',
                    reviewer_solution_hi: solutionsHi[linkId] || '',
                    subtype: subtypes[linkId] || 'other',
                    exam_name: selectedPaper?.exam_name || '',
                    section: pair.section_code || '',
                }),
            });
            const data = await res.json();
            if (res.ok && data.success) {
                setAiResults(prev => ({ ...prev, [linkId]: data.parsed }));
    ...
    }, [selectedOptions, solutionsEn, solutionsHi, subtypes, selectedPaper]);

    const handleSave = useCallback(async (pair) => {
        const linkId = pair.link_id;
        const aiResult = aiResults[linkId];
        const answerLabel = selectedOptions[linkId];
        ...
        if (!aiResult) {
            setFeedback({ type: 'error', message: 'Generate AI enrichment first.' });
            return;
        }
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                setPairs(prev => prev.map(p =>
                    p.link_id === linkId
                        ? { ...p, solution_status: 'DONE', correct_option_label: answerLabel, difficulty: aiResult.difficulty }
                        : p
                ));
            } else {
    ...
    }, [aiResults, selectedOptions, subtypes]);

    const handleFlag = useCallback(async (pair) => {
        const linkId = pair.link_id;
        setFlaggingId(linkId);
        setFeedback(null);
        try {
            const res = await fetch('/api/image-solutions-bilingual/save', {
                method: 'POST',
            ...
            const data = await res.json();
            if (res.ok && data.success) {
                setFeedback({ type: 'success', message: `Q.${pair.eng_source_no || linkId} flagged.` });
                setPairs(prev => prev.map(p =>
                    p.link_id === linkId ? { ...p, link_status: 'FLAGGED' } : p
                ));
            } else {
                ...
                {!selectedPaper ? (
                    <div className="flex items-center justify-center h-full">
                        <div className="text-center">
                            ...
                            <p className="text-gray-400 mt-2 text-sm">Choose a paper from the left to review bilingual image questions.</p>
                        </div>
                    </div>
                ) : (
                    <div>
                            ...
                            {loadingPairs ? (
                                ...
                                filteredPairs.map((pair, idx) => (
                                    <PairCard
                                        ...
                                        aiResult={aiResults[pair.link_id]}
        ...
        <div className="grid grid-cols-4 gap-2">
            {(options || []).map(opt => {
                ...
                return (
                    <button
                        key={opt.opt_label}
                        onClick={() => onSelect(opt.opt_label)}
...
function PairCard({ pair, idx, selectedOption, onSelectOption, subtype, onSubtypeChange, solutionEn, onSolutionEnChange, solutionHi, onSolutionHiChange, aiResult, isGenerating, isTranslating, isSaving, isFlagging, onTranslate, onGenerate, onSave, onFlag }) {
        ...
        <div className={`bg-white rounded-lg border shadow-sm overflow-hidden ${isFlagged ? 'border-red-300' : isSolved ? 'border-green-300' : 'border-gray-200'}`}>
            ...
            <div className={`px-5 py-3 border-b flex items-center justify-between ${isFlagged ? 'bg-red-50 border-red-200' : isSolved ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100'}`}>
                <div className="flex items-center gap-2">
                    ...
                    <DifficultyBadge level={pair.difficulty || aiResult?.difficulty} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-gray-200">
                ...
                <div className="p-5 space-y-3">
                    ...
                    <div className="text-sm text-gray-800">
                        <Latex>{pair.eng_text || '(No text)'}</Latex>
                    </div>
                ...
                <div className="p-5 space-y-3">
                    ...
                    <div className="text-sm text-gray-800">
                        <Latex>{pair.hin_text || '(No Hindi text)'}</Latex>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-gray-200 border-t border-gray-100">
                <div className="p-5">
                    ...
                    <textarea
                        rows={3}
                        value={solutionEn}
                        onChange={e => onSolutionEnChange(e.target.value)}
                ...
                <div className="p-5">
                    <div className="flex items-center justify-between mb-1.5">
                        ...
                        <button
                            onClick={onTranslate}
                            disabled={isTranslating || !solutionEn}
                    ...
                    <textarea
                        rows={3}
                        value={solutionHi}
                        onChange={e => onSolutionHiChange(e.target.value)}
            ...
            {aiResult?.full_json?.display_sections && (
                <div className="px-5 py-3 border-t border-gray-100">
                    <div className="flex items-center gap-4 p-3 bg-blue-50 rounded-lg border border-blue-200 text-sm">
                        ...
                        {aiResult.full_json.subtype && (
                            <div>
                                <span className="text-xs text-gray-500 uppercase">Subtype</span>
                                <div className="text-sm font-medium text-gray-700">{aiResult.full_json.subtype}</div>
                            </div>
                        )}
                        <div className="flex-1">
                            <span className="text-xs text-gray-500 uppercase">Key Concept</span>
                            <div className="text-xs text-gray-700">{aiResult.full_json.display_sections?.exam_craft?.key_concept || '-'}</div>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="px-5 py-3 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        onClick={onSave}
                        disabled={isSaving || !selectedOption || !aiResult}
                    ...
                    >
                        {isSaving ? (
                            <><span className="animate-spin inline-block w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full"></span>Saving...</>
                        ) : 'Save'}
                    </button>
                </div>
            </div>
