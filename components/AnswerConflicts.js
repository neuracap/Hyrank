...
function SolutionPanel({ item, sol, coreBasis, finalAnswerText, solutionCorrectOption, displaySections, figureUrl, onSaved }) {
    ...
    const [draftClearFigure, setDraftClearFigure] = useState(false);
    ...
    const saveEdit = async () => {
        ...
        try {
            ...
            if (draftClearFigure) body.clear_figure = true;
            ...
            <div className="p-4 text-sm text-gray-700 space-y-3">
                {editing ? (
                    <>
                        ...
                        {figureUrl && (
                            <FieldEdit label="Solution figure" hint="Generated by the solver. Tick 'Remove figure' if it's wrong/junk.">
                                <div className={`border rounded p-2 bg-white ${draftClearFigure ? 'border-red-300 bg-red-50' : 'border-gray-200'}`}>
                                    <div className="flex items-start gap-3">
                                        <img src={figureUrl} alt="Solution figure"
                                            className={`max-h-48 rounded border border-gray-200 object-contain ${draftClearFigure ? 'opacity-40 grayscale' : ''}`} />
                                        <div className="flex-1 space-y-2">
                                            <label className="flex items-center gap-2 text-sm">
                                                <input type="checkbox" checked={draftClearFigure}
                                                    onChange={e => setDraftClearFigure(e.target.checked)} />
                                                <span className={`font-semibold ${draftClearFigure ? 'text-red-700' : 'text-gray-700'}`}>
                                                    Remove this figure on save
                                                </span>
                                            </label>
                                            <div className="text-[11px] text-gray-500 break-all">
                                                URL: <span className="font-mono">{figureUrl}</span>
                                            </div>
                                            {draftClearFigure && (
                                                <div className="text-[11px] text-red-700 italic">
                                                    Will clear answer_outcome.figure_url from the saved solution.
                                                </div>
                                            )}
                                        </div>
                                    </div>
